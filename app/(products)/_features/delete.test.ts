...
Given(
  "a delete button represented by a cross icon for each product",
  async function () {
...
When(
  "the user clicks the delete button for a specific product",
  async function () {
...
Then(
  "the specific product is removed from the paginated list",
  async function () {
    await driver.wait(
      until.elementIsNotVisible(
        driver.findElement(By.css(PRODUCT_LIST_SELECTOR + ":first-child"))
      )
    );
  }
);
...
Then(
  "the selected products are removed from the paginated list",
  async function () {
    await driver.wait(
      until.elementsLocated(By.css(PRODUCT_LIST_SELECTOR)),
      10000
    );
